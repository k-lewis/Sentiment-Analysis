...
% loop through parameter values and find pair with best CCR
for n = 1:21
    for i = 1:17
        svm_train = svmtrain(vertcat(CV_1, CV_2, CV_3, CV_4), vertcat(CV_1_labels, CV_2_labels, CV_3_labels, CV_4_labels),'kernel_function','rbf','autoscale','false','kernelcachelimit',500000,'boxconstraint',C_values(n)*ones(1280,1), 'rbf_sigma',rbf_sigma_values(i));
        svm_classify = svmclassify(svm_train, CV_5);
        clear svm_train
        CCRs(1) = sum(svm_classify == CV_5_labels)/length(svm_classify);
        clear svm_classify
        
        svm_train = svmtrain(vertcat(CV_1, CV_2, CV_3, CV_5), vertcat(CV_1_labels, CV_2_labels, CV_3_labels, CV_5_labels),'kernel_function','rbf','autoscale','false','kernelcachelimit',500000,'boxconstraint',C_values(n)*ones(1280,1), 'rbf_sigma',rbf_sigma_values(i));
        svm_classify = svmclassify(svm_train, CV_4);
        clear svm_train
        CCRs(2) = sum(svm_classify == CV_4_labels)/length(svm_classify);
        clear svm_classify
        
        svm_train = svmtrain(vertcat(CV_1, CV_2, CV_5, CV_4), vertcat(CV_1_labels, CV_2_labels, CV_5_labels, CV_4_labels),'kernel_function','rbf','autoscale','false','kernelcachelimit',500000,'boxconstraint',C_values(n)*ones(1280,1), 'rbf_sigma',rbf_sigma_values(i));
        svm_classify = svmclassify(svm_train, CV_3);
        clear svm_train
        CCRs(3) = sum(svm_classify == CV_3_labels)/length(svm_classify);
        clear svm_classify
        
        svm_train = svmtrain(vertcat(CV_1, CV_5, CV_3, CV_4), vertcat(CV_1_labels, CV_5_labels, CV_3_labels, CV_4_labels),'kernel_function','rbf','autoscale','false','kernelcachelimit',500000,'boxconstraint',C_values(n)*ones(1280,1), 'rbf_sigma',rbf_sigma_values(i));
        svm_classify = svmclassify(svm_train, CV_2);
        clear svm_train
        CCRs(4) = sum(svm_classify == CV_2_labels)/length(svm_classify);
        clear svm_classify
        
        svm_train = svmtrain(vertcat(CV_5, CV_2, CV_3, CV_4), vertcat(CV_5_labels, CV_2_labels, CV_3_labels, CV_4_labels),'kernel_function','rbf','autoscale','false','kernelcachelimit',500000,'boxconstraint',C_values(n)*ones(1280,1), 'rbf_sigma',rbf_sigma_values(i));
...
svm_train = svmtrain(vertcat(CV_1, CV_2, CV_3, CV_4, CV_5), vertcat(CV_1_labels, CV_2_labels, CV_3_labels, CV_4_labels, CV_5_labels), 'kernel_function','rbf', 'autoscale','false','kernelcachelimit',500000,'boxconstraint',C_star*ones(1600,1), 'rbf_sigma',rbf_sigma_star);
